...
class Application:
    ...
    def __init__(self):
        ...
        # Create a frame for the three main panels
        panel_frame = tk.Frame(self.root)
        panel_frame.place(x=50, y=60)

        # Webcam feed panel
        self.panel = tk.Label(panel_frame)
        self.panel.grid(row=0, column=0, padx=10)
        self.panel.config(width=480, height=400)

        # Hand gesture panel
        self.panel2 = tk.Label(panel_frame)
        self.panel2.grid(row=0, column=1, padx=10)
        self.panel2.config(width=480, height=400)

        # ASL reference image
        try:
            asl_img = Image.open('Alphabet.jpg')
            asl_img = asl_img.resize((480, 400), Image.Resampling.LANCZOS)
            self.asl_photo = ImageTk.PhotoImage(asl_img)
            self.asl_panel = tk.Label(panel_frame, image=self.asl_photo)
            self.asl_panel.grid(row=0, column=2, padx=10)
        except Exception as e:
            print(f"Error loading ASL reference image: {e}")

        # Text display frame
        text_frame = tk.Frame(self.root)
        text_frame.place(x=50, y=500)

        # Character display
        self.T1 = tk.Label(text_frame, text="Character:", font=("Courier", 30, "bold"))
        self.T1.grid(row=0, column=0, sticky='w', pady=10)
        self.panel3 = tk.Label(text_frame)
        self.panel3.grid(row=0, column=1, sticky='w', padx=20)

        # Sentence display
        self.T3 = tk.Label(text_frame, text="Sentence:", font=("Courier", 30, "bold"))
        self.T3.grid(row=1, column=0, sticky='w', pady=10)
        self.panel5 = tk.Label(text_frame)
        self.panel5.grid(row=1, column=1, sticky='w', padx=20)

        # Control buttons
        button_frame = tk.Frame(text_frame)
        button_frame.grid(row=1, column=2, padx=20)
        
        self.clear = tk.Button(button_frame, text="Clear", font=("Courier", 20), command=self.clear_fun)
        self.clear.pack(side=tk.LEFT, padx=5)
        
        self.speak = tk.Button(button_frame, text="Speak", font=("Courier", 20), command=self.speak_fun)
        self.speak.pack(side=tk.LEFT, padx=5)

        # Suggestions section
        self.T4 = tk.Label(text_frame, text="Suggestions:", fg="red", font=("Courier", 30, "bold"))
        self.T4.grid(row=2, column=0, sticky='w', pady=10)

        suggestion_frame = tk.Frame(text_frame)
        suggestion_frame.grid(row=2, column=1, sticky='w')

        # Suggestion buttons
        self.b1 = tk.Button(suggestion_frame)
        self.b1.pack(side=tk.LEFT, padx=5)
        
        self.b2 = tk.Button(suggestion_frame)
        self.b2.pack(side=tk.LEFT, padx=5)
        
        self.b3 = tk.Button(suggestion_frame)
        self.b3.pack(side=tk.LEFT, padx=5)
        
        self.b4 = tk.Button(suggestion_frame)
